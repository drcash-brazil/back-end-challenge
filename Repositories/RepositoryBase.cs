            ...
            public class RepositoryBase<T> : IRepositoryBase<T> where T : Base
            {
                        ...
                        public async Task<Response> add(T model)
                        {
                                    try
                                    {
                                                ...
                                                return new Response { data = model, status = true, message = "" };

                                    }
                                    catch (Exception ex)
                                    {
                                                return new Response { errors = new List<string> { ex.InnerException.Message }, data = null, status = false, message = "" };
                                    }
                        }
                        ...
                        public async Task<Response> delete(string id)
                        {
                                    try
                                    {
                                                ...
                                                return new Response { data = null, status = true, message = "" };
                                    }
                                    catch (Exception ex)
                                    {
                                                return new Response { errors = new List<string> { ex.InnerException.Message }, data = null, status = false, message = "" };
